...
The bot cannot start without the main Telegram token:
...
Set these as well when the corresponding features are enabled:

```text
OWNER_ID=<numeric Telegram user ID>
NOWPAYMENTS_API_KEY=<NowPayments API key used to create addresses and QR codes>
SERVER_URL=https://your-public-domain.example
PORT=5000
```

`OWNER_ID` is strongly recommended because the code has a legacy fallback.
`NOWPAYMENTS_API_KEY` is required to create deposit addresses/QR codes.
Deposits are detected and credited by the independent blockchain address
monitor; NOWPayments IPN callbacks are intentionally ignored for settlement.
`SERVER_URL` is only required for any separately enabled public callback
feature. Dealer-bot, Plisio, Cryptomus, Telegram Stars, group, blockchain
endpoint overrides, and 2FA variables are optional and are listed in
`bot.env.example`.
