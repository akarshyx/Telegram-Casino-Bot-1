...
EVM_TOKEN_CONTRACTS = {
    ...
    ("BSC", "DAI"): ("0x1af3f329e8be154074d8769d1ffa4ee058b1dbc3", 18),
    ...
    try:
        ...
        for tx in entries if isinstance(entries, list) else []:
            ...
            if contract.get("type") != "TransferContract" or value.get("to_address") != address:
                continue
            # TronGrid returns failed transactions in the same account feed.
            # Never turn a reverted/aborted transfer into a deposit.
            ret = tx.get("ret") or []
            if isinstance(ret, list) and any(
                isinstance(item, dict)
                and str(item.get("contractRet") or "").upper() not in {"", "SUCCESS"}
                for item in ret
            ):
                continue
            ...
            confirmations = (
                max(0, current_number - block_number + 1)
                if block_number and current_number
                else 0
            )
            item = _make_tx(
                txid=tx.get("txID"),
...
def _ton_headers() -> dict[str, str]:
    api_key = os.environ.get("DEPOSIT_TON_API_KEY", "").strip()
    return {"X-API-Key": api_key} if api_key else {}


def _ton_transaction_succeeded(tx: dict[str, Any]) -> bool:
    """Reject aborted/failed TON transactions returned by indexers."""
    description = tx.get("description") or {}
    if not isinstance(description, dict):
        description = {}
    if description.get("aborted") is True:
        return False
    compute = description.get("compute_phase") or {}
    if isinstance(compute, dict) and not compute.get("skipped"):
        if _int(compute.get("exit_code")) not in {0}:
            return False
    action = description.get("action_phase") or {}
    if isinstance(action, dict) and _int(action.get("result_code")) not in {0}:
        return False
    return True


def _scan_ton(deposit: dict[str, Any], coin: str) -> AddressScan:
    """Scan finalized TON and Jetton transfers sent to the exact payment address.

    TON indexers expose finalized transactions rather than a portable
    confirmation-height API.  A successful indexed transaction is therefore
    treated as final, while the persisted required-confirmations value is still
    used by the common monitor contract.
    """
    address = _address(deposit.get("pay_address") or deposit.get("deposit_address"))
    required = _required_confirmations(deposit)
    base = os.environ.get(
        "DEPOSIT_TON_API_URL", "https://toncenter.com/api/v2"
    ).rstrip("/")
    headers = _ton_headers()
    try:
        # Native Toncoin is returned by some provider integrations as TON and
        # by older integrations as GRAM. Both must use the account transaction
        # endpoint; GRAM is not a Jetton contract.
        if coin in {"TON", "GRAM"}:
            payload = _get_json(
                f"{base}/getTransactions",
                params={"address": address, "limit": 100, "archival": "true"},
                headers=headers,
            )
            entries = payload.get("result", []) if isinstance(payload, dict) else []
            result: list[ChainTransaction] = []
            for tx in entries if isinstance(entries, list) else []:
                if not isinstance(tx, dict) or not _ton_transaction_succeeded(tx):
                    continue
                incoming = tx.get("in_msg") or {}
                if not isinstance(incoming, dict):
                    continue
                destination = incoming.get("destination") or tx.get("account")
                if not _same_address(destination, address):
                    continue
                amount = _decimal(incoming.get("value")) / Decimal(1_000_000_000)
                transaction_id = tx.get("transaction_id") or {}
                txid = (
                    transaction_id.get("hash")
                    if isinstance(transaction_id, dict)
                    else transaction_id
                ) or tx.get("hash") or tx.get("lt")
                item = _make_tx(
                    txid=txid,
                    address=destination,
                    amount=amount,
        ...
        # Jetton transfer history is served by Toncenter v3.  The endpoint
        # returns the jetton symbol and decimals, so the selected token is
        # checked instead of crediting an unrelated token sent to the address.
        jetton_base = os.environ.get(
            "DEPOSIT_TON_JETTON_API_URL", "https://toncenter.com/api/v3"
        ).rstrip("/")
        payload = _get_json(
            f"{jetton_base}/jetton/transfers",
            params={"address": address, "direction": "in", "limit": 100},
            headers=headers,
        )
        entries = (
            payload.get("jetton_transfers", payload.get("transfers", []))
        ...
        for row in entries if isinstance(entries, list) else []:
            ...
            if not _same_address(destination, address):
                continue
            jetton = row.get("jetton") or row.get("jetton_master") or {}
            if isinstance(jetton, dict):
                symbol = str(jetton.get("symbol") or "").upper()
                decimals = _int(jetton.get("decimals"), 6)
            else:
                symbol = str(row.get("jetton_symbol") or "").upper()
                decimals = _int(row.get("decimals"), 6)
            if symbol and symbol != coin:
                continue
            raw_amount = _decimal(row.get("amount") or row.get("value"))
            txid = (
        ...
        return AddressScan(error=f"{coin}/TON explorer error: {exc}")
